...
export class MessagesService {
  allMessages = messages.filter(
    (message) => message.to === 'devguy@angular.dev'
  );
  ...
  getAllUsers(allUsers) {
    messages.forEach(message => allUsers.add(message.to));
  }

  getFolder(folder: string) {
    return this.allMessages.filter((message) => message.folder === folder);
  }

  getMessageID(id: string) {
    return this.allMessages.filter((message) => message._id === id);
  }
}
